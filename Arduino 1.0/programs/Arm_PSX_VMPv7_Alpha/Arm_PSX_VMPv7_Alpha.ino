...
void setup()
{
  ...
  setArmTo( x = 0, y = -76.00, z = 652.00, gripperAngle = 0 ); //Use (y=-76, z=652) for long, (y=0, z=680) for medium, (y =-200, z=350) for no extension
  myssc.servoMove( WRO_SERVO, wristRotate = 1500 );
  myssc.servoMove( GRI_SERVO, gripperServo = 890 );
...
void loop()
{
  ...
  setArmTo( x, y, z, gripperAngle );
  //Update to Wrist and Grip positions
  myssc.servoMove( WRO_SERVO, wristRotate );
  myssc.servoMove( GRI_SERVO, gripperServo );
...
byte PSX_poll( void )
{
  ...
  if(ps2x.Button(PSB_R1))
  {
    ...
    if(ps2x.Button(PSB_L3)){
      ...
      z = 232;
      gripperAngle = -29;
      myssc.servoMove( WRO_SERVO, wristRotate = 1200 );
    }
...
void setArmTo( float x, float y, float z, float grip_angle_d )
{
