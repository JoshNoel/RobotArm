...
void setup()
{
  ...
  setArmTo( x = 0, y = -76.0, z = 652.0, wristAngle = 0, wristRotateServoPulse = CENTER, gripperOpeningServoPulse = CENTER-610 );
...
byte PSX_poll( void )
{
  ...
  if(ps2x.Button(PSB_L3))  gripperOpeningServoPulse = CENTER-610; // Open the gripper all the way with left analog stick click
  // Gripper opening stop points
  if(gripperOpeningServoPulse < CENTER-1000 )  gripperOpeningServoPulse = CENTER-1000;
  if(gripperOpeningServoPulse > CENTER+1100 )  gripperOpeningServoPulse = CENTER+1100;
